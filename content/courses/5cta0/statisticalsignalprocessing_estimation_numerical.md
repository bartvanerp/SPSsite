...
\end{equation}
where $s[n,\theta]$ is the signal model and $\theta_0$ is the parameter value for which we are calculating the approximation. This approximation for the scalar parameter $\theta$ is extended to vector parameter $\Theta$:
\begin{equation}
\mathbf{s}(\Theta)\approx \mathbf{s}(\Theta_0)+\mathbf{H}(\Theta_0)(\Theta-\Theta_0),
\end{equation}
where the observation matrix $\mathbf{H}(\Theta_0)$ consists of columns $\mathbf{h}_k$, i.e., $\mathbf{H}(\Theta_0)=[\mathbf{h}_1~ \mathbf{h}_2~...~\mathbf{h}_K]$,
\begin{equation}
\mathbf{h}_k=\frac{\partial \mathbf{s}(\Theta)}{\partial\theta_k}.
\end{equation}
Thus, the partial derivative of the signal model $\mathbf{s}(\Theta)$ with respect to an element $\theta_k$ of the parameter vector $\Theta$ is evaluated at $\Theta=\Theta_0$ and the values constitute the $k^{th}$ column of the observation matrix.
...
\begin{equation}
\hat\Theta=\Theta_0+\big(\mathbf{H}(\Theta_0)^T \mathbf{H}(\Theta_0)\big)^{-1}\mathbf{H}(\Theta_0)^T\big(\mathbf{x}-\mathbf{s}(\Theta_0)\big)
\end{equation}

We must note that the estimate $\hat\Theta$ depends on our choice of initial value $\Theta_0$. We can cast this approximate estimation into an iterative form, where the $m+1^{st}$ iteration step is:
\begin{equation}
\Theta_{m+1}=\Theta_m+\big(\mathbf{H}(\Theta_m)^T \mathbf{H}(\Theta_m)\big)^{-1}\mathbf{H}(\Theta_m)^T\big(\mathbf{x}-\mathbf{s}(\Theta_m)\big)
\end{equation}
Starting from an initial value $\Theta_0$, the LSE can be iteratively calculated to converge to a minimum value. Of course, there has to be <i>criteria to stop the iterations</i>, such as limited change in the parameter estimate, i.e., $(\Theta_{m+1}-\Theta_m)^T(\Theta_{m+1}-\Theta_m)<\epsilon$, and reaching a maximum number of iterations. Selection of the initial parameter vector $\Theta_0$ can be based on a grid search over the squared error function to make sure the numerical solution begins close to the global maximum.
...
We consider the Newton-Raphson method for <a href="../estimation_maximumlikelihood">the maximum likelihood estimation (MLE)</a>, where set the first derivative of the likelihood function to zero and solve it for the parameter vector $\Theta$. The Newton-Raphson method is used to numerically calculate the parameter value at which the derivative of the log-likelihood function is zero. Thus, we will be dealing with both the first and the second derivative of the log-likelihood function. The iterative MLE for the parameter vector $\Theta$ is
\begin{equation}
\Theta_{m+1}=\Theta_m-\frac{\partial \ln p(\mathbf{x};\Theta)}{\partial\Theta}\Bigg|_{\theta=\theta_m}\Bigg(\frac{\partial^2 \ln p(\mathbf{x};\Theta)}{\partial\Theta^2}\Bigg|_{\theta=\theta_m}\Bigg)^{-1}.
\end{equation}
...
\begin{equation}
\Theta_{m+1}=\Theta_m+\mathcal{I}^{-1}(\Theta_m)\frac{\partial \ln p(\mathbf{x};\Theta)}{\partial\Theta}\Bigg|_{\theta=\theta_m}.
\end{equation}
...
\begin{equation}
J(\Theta)=(\mathbf{x}-\mathbf{H}\Theta)^T\mathbf{C}^{-1}(\mathbf{x}-\mathbf{H}\Theta),
\end{equation}
...
\begin{equation}
\hat\Theta=(\mathbf{H}^T\mathbf{C}^{-1}\mathbf{H})^{-1}\mathbf{H}^T\mathbf{C}^{-1}\mathbf{x}.
\end{equation}
