...
2. <a href="../statisticalsignalprocessing_signals_approx">Approximate signal statistics</a> - In all practical applications, the exact signal statistics are unknown. These therefore need to be approximated based on a limited set of data.
3. <a href="../statisticalsignalprocessing_signals_second">Second moment analysis</a> - The second moment of a random signal can be analyzed in order to obtain some interesting results. As it turns out through the Wiener-Khinchin theorem it follows that the power spectral density of a random signal is given by the Fourier transform of the auto-correlation function.
