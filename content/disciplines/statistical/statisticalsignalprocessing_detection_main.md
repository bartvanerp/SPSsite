...
Another example of detection theory involves radar or sonar, where the goal is to identify flying or submerged objects. In the case of radar, we might, for example, be interested in detecting a hostile aircraft. The detection of such an aircraft is of vital importance. In case of its presents, an appropriate reaction needs to be taken. However, if an aircraft is detected by the system and there actually is not one, then the consequences of performing a military counter-operation might cause a disastrous international conflict.
