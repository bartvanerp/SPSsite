...
To illustrate the consequences of windowing, we will illustrate the case of the simplest window: the rectangular window. The rectangular window $w[n]$ can be defined as
\begin{equation}
    w[n] =
    \begin{cases}
...
Suppose that we know that the Fourier transform of discrete-time signals of $x[n]$ is ideally $X(e^{j\theta})$ and we would like to know what the effect of windowing is on the estimated spectrum. Recall that a convolution in the time-domain results in a multiplication in the frequency domain. It can be shown that the opposite relation holds as well: a multiplication in the time domain will result in a periodic convolution in the frequency domain. This relationship leads to the following frequency response of the windowed signal $\tilde{X}(e^{j\theta})$ as
\begin{equation}
...
\begin{equation}
    ...
    \end{split}
\end{equation}
where the ratio $\frac{\sin(N\theta/2)}{\sin(\theta/2)}$ is the <i>Dirichlet</i> or <i>periodic and aliased sinc</i> function. This function is convoluted with the true spectrum $ X(e^{j\theta}) $ to obtain the estimated spectrum $\tilde{X}(e^{j\theta})$, obtained from the windowed signal $\tilde{x}[n]$. The frequency response of the rectangular window is also a function of the window length $N$. Fig. 1 shows three rectangular windows with their respective normalized magnitude responses on the logarithmic scale for increasing $N$. The magnitude response is usually normalized to the peak at 0 dB for comparison. It can be noted from the figure that an increase of $N$ results in a better <i>resolution</i> of the estimated spectrum. However, the height of the side lobe does not change with $N$. This has important consequence for spectral leakage, as will be discussed hereafter.
