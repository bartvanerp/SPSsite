...
\hline
Type & \multicolumn{2}{|c|}{Phase jump of $\pi$ at} & Comment \\
...
&\overset{l=-k+M-1}{=} &
\sum_{l=M-1}^{0} h_{M-1-l} z^{-l} \\ &\overset{h_{M-1-l}=h_l}{=}& \sum_{l=0}^{M-1} h_l z^{-l} = H(z).
\end{eqnarray*}
...
\begin{eqnarray*}
H_{II}(z)\vert_{z=-1} &=& \left \{ z^{-(M-1)} H_{II}(z^{-1}) \right \} \left . \right \vert_{z=-1} \\
...
\begin{tabular}{|c|c|c|}
\hline
Type & $z=1$ & $z=-1$ \\ \hline \hline
I & -- & -- \\ \hline
II & -- & $H_{II}(-1)=0$ \\ \hline
III & $H_{III}(1)=0$ & $H_{III}(-1)=0$ \\ \hline
IV & $H_{IV}(1)=0$ & -- \\
\hline
\end{tabular}
\end{center}
From the fact that $z=1$ coincides with $\theta=0$ and $z=-1$ with $\theta=\pm \pi$, it follows that these results are a generalized representation of the results as found in the previous sub section.
...
\\[2ex]
\example{The equation shows the mathematical expression of the frequency response of an odd length 5 FIR filter with impulse response $h_I[n]$:
$$
...
\begin{center}
\includegraphics[scale=0.4]{figs/MagPhaseTypeI.pdf}
\includegraphics[scale=0.4]{figs/PZplotTypeI.pdf}
\end{center}
The system function writes as follows:
\begin{eqnarray*}
H_I(z)&=& {\color{red}1} {\color{blue}-3} z^{-1} + \frac{9}{2} z^{-2} {\color{blue}-3} z^{-3}{\color{red}+} z^{-4} \\
&=&
(1- z_1 z^{-1}) \cdot (1- z^*_1 z^{-1}) \cdot (1- (1/z_1) z^{-1}) \cdot(1- (1/z^*_1) z^{-1})
\end{eqnarray*}
...
\text{ }\\[2ex]
\example{The equation shows the mathematical expression of the frequency response of an odd length 5 FIR filter with impulse response $h[n]$:
$$
...
&=&2 {\color{green}\cj}  \cdot \Big(  {\color{red}h_0} \sin(2 \theta) + {\color{blue}h_1} \sin(\theta)\Big) \cdot e^{-j2 \theta}\\
...
\begin{center}
\includegraphics[scale=0.4]{figs/MagPhaseTypeIII.pdf}
\includegraphics[scale=0.4]{figs/PZplotTypeIII.pdf}
\end{center}
...
\\[2ex]
\example{Give a realization scheme of the type II even length $M$ even-symmetric linear-phase filter, thus $h[M-1-n]=h[n]$, as depicted in Fig. \ref{Fig:directform} with a reduction of the number of multiplications with 50 \%}
...
\begin{center}
\includegraphics[scale=0.7]{figs/evensymmetry.pdf}
\end{center}
...
\\[2ex]
\example{Show that the following system is all pass:
$$
...
\Rightarrow & & |H(e^{j\theta})| = \vertH(z)\vert_{|z|=1} =1
\end{eqnarray*}
...
\begin{center}
\includegraphics[scale=0.4]{figs/AllpassPZ.pdf}
\includegraphics[scale=0.4]{figs/AllpassAmplPhase.pdf}
\end{center}
...
A stable and causal filter that has a stable and causal inverse is said to have minimum phase and so a minimum phase system has all of its poles and zeros inside the unit circle, as shown in Fig. \ref{Fig:minphase}.
\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.4]{figs/pzminphase.pdf}
\includegraphics[scale=0.4]{figs/magphaseminphase.pdf}
\end{center}
\caption{Pole-zero plot and magnitude- and phase-response of minimum phase system}
\label{Fig:minphase}
\end{figure}
The magnitude and phase response of a minimum phase system with three zeros inside the unit circle is depicted in Fig. \ref{Fig:minphase}.

When mirroring all zeros of a stable and causal minimum phase system we obtain a maximum phase system and so a maximum phase system has all of its poles inside and its zeros outside the unit circle.
, as shown in Fig. \ref{Fig:maxphase}.
\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.4]{figs/pzmaxphase.pdf}
\includegraphics[scale=0.4]{figs/magphasemaxphase.pdf}
\end{center}
\caption{Pole-zero plot and magnitude- and phase-response of maximum phase system.}
\label{Fig:maxphase}
\end{figure}
The magnitude and phase response of a minimum phase system with three zeros inside the unit circle is depicted in Fig. \ref{Fig:maxphase}
...
{\em Notes:}
\begin{itemize}
\item Mirroring a zero with respect to the unit circle does
not change the shape of the magnitude response. Thus both minimum and maximum phase systems have, besides a constant factor, the same shape of the magnitude response characteristic.
\item The phase of a minimum phase system has
less variations compared to the phase variations of a maximum phase system.
\item The group delay is defined as:
$$
\tau (\theta) = - \frac{\text{d} \varphi \{ H(e^{j\theta}) \}}{\text{d} \theta},
$$
The group delay of a minimum phase system is minimal.A s\end{itemize}
...
\begin{enumerate}
\item First mirror the zeros which are outside $|z|=1$ into
new zeros inside $|z|=1$.
\item Then cancel the new zeros by new poles.
\item Combine the old and new zeros that are inside $|z|=1$
and the old poles together which results in a minimum phase system.
\item The combination of the old zeros that are outside
$|z|=1$ and the new poles results in an all pass system.
\end{enumerate}
\text{ } \\[2ex]
\example{Factor the following system function as the product of a minimum phase system and an all-pass system:
$$
...
\begin{center}
\includegraphics[scale=1.0]{figs/ExampleFact.pdf}
\end{center}
...
\begin{center}
\includegraphics[scale=0.4]{figs/PZCombN1.pdf}
\includegraphics[scale=0.4]{figs/AmplPhasCombN1.pdf}
\end{center}
...
\begin{center}
\includegraphics[scale=0.4]{figs/PZCombN5.pdf}
\includegraphics[scale=0.4]{figs/AmplPhasCombN5.pdf}
\end{center}
...
<br></br>
## Averaging filter
An $M$ point averaging filter can be realized with the following non recursive direct form I structure as a transversal FIR with unit weights as depicted in Fig. \ref{Fig:averagefilFIR}.
\begin{figure}[h]
\centering
\includegraphics[scale=0.9]{figs/averagefilFIR.pdf}
\caption{$M$ point averaging FIR filter.}
\label{Fig:averagefilFIR}
\end{figure}
...
\end{eqnarray*}
from which the magnitude response has its main lobe around frequency zero and the phase response is linear as shown in Fig. \ref{Fig:DirichletN21}.
\begin{figure}[h]
\centering
\includegraphics[scale=0.6]{figs/DirichletM21.pdf}
\caption{Dirichlet function: Magnitude- and Phase- response of $M=21$ averaging filter.}
\label{Fig:DirichletN21}
\end{figure}
...
\text{ } \\[2ex]
\example{Give the realization scheme of an averaging filter with $M=5$ as the cascading of a fifth order non recursive structure  filter and a first order recursive structure. Also give the pole zero pot and a sketch of the magnitude response}
{
$$
...
\begin{center}
\includegraphics[scale=0.8]{figs/averagefilCasc.pdf}
\end{center}
...
\begin{center}
\includegraphics[scale=0.4]{figs/pzplotaverageN5.pdf}
\includegraphics[scale=0.4]{figs/AmplDirichletN5.pdf}
\end{center}}

<br></br>
## Frequency sampling filter
The previously discussed philosophy of the averaging filter
can be extended to an $M$ point transversal FIR filter with weights $h_k$ as shown in Fig. \ref{Fig:FIR}
\begin{figure}[h]
\centering
\includegraphics[scale=0.9]{figs/FIR.pdf}
\caption{$M$ point transversal FIR filter.}
\label{Fig:FIR}
\end{figure}
...
\end{eqnarray*}
A realization scheme of this frequency sampling filter structure is depicted in Fig. \ref{Fig:Freqsampl}.
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figs/Freqsampl.pdf}
\caption{Structure Frequency sampling filter.}
\label{Fig:Freqsampl}
\end{figure}
...
\text{ } \\[2ex]
\example{Design a frequency sampling filter from which the frequency response is defined for the following frequencies:
$$
H(e^{j\theta}) =
\left \{
\begin{array}{cl}
...
\left ( \frac{3}{1 -z^{-1} \alpha_1} \right )
+
\left ( \frac{3}{1 -z^{-1} \alpha_1^\ast} \right )}
\right \}
\end{eqnarray*}
...
\begin{center}
\includegraphics[scale=0.9]{figs/Freqsampl1.pdf}
\end{center}
...
\begin{center}
\includegraphics[scale=0.9]{figs/Freqsampl2.pdf}
\end{center}
...
\begin{center}
\includegraphics[scale=0.4]{figs/PZFreqsamplN5.pdf}
\includegraphics[scale=0.4]{figs/FreqsamplN5.pdf}
\end{center}
...
<br></br>
## Lattice filter
A lattice filter has a modular structure. That is a lattice
consists of a number of cascaded modules and each module has the same structure. For example, the top of Fig. \ref{Fig:LatticeFIR} shows a typical module of a lattice filter, which is a two port, with two inputs, two outputs, one delay and two multipliers with a factor $\Gamma_k$.
\begin{figure}[h]
\centering
\includegraphics[scale=0.9]{figs/LatticeFIR.pdf}
\caption{Lattice FIR filter.}
\label{Fig:LatticeFIR}
\end{figure}
In most literature these parameters are denoted by reflection coefficients.
Since the module does not contain a feedback path the cascading of $p$ of such modules represent a $p^{th}$ order FIR filter, as shown at the bottom of Fig. \ref{Fig:LatticeFIR}.

In a similar way lattice filters can be used to implement All pole structures or structures with poles and zeros, as depicted in Fig. \ref{Fig:LatticeAPIIR}.
\begin{figure}[h]
\hspace*{-10mm}
\includegraphics[scale=1.0]{figs/LatticeAP.pdf}
\\[4ex]
\hspace*{-10mm}
\includegraphics[scale=0.9]{figs/LatticeIIR.pdf}
%\end{center}
\caption{Lattice All Pole (top) and IIR filter (bottom).}
\label{Fig:LatticeAPIIR}
\end{figure}
...
\end{eqnarray*}
which represents the  non recursive second order transversal structure as depicted in Fig. \ref{Fig:LatticeExample}.
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figs/LatticeExample.pdf}
\caption{Example of Lattice structure of a non recursive second order transversal filter.}
